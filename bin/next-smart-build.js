...
async function main() {
  ...
  if (symlinked.length > 0) {
    ...
    console.log('➡️   Using Webpack: running `next build --webpack`')
    args.push('--webpack')
  } else {
    console.log('⚡  No symlinked packages detected; using Turbopack via `next build`')
  }

  runNext(command, [...args, ...forwardArgs])
}
